...
module.exports.handleEvent = async function({ api, event, client, __GLOBAL }) {
  ...
  if (haha.includes("raniel") || haha.includes("")) {
  ...
  if (event.logMessageType == "log:unsubscribe") {
    ...
    const userName = await global.GoatBot.Users.getName(leftParticipantFbId); // Access user data from the GoatBot API

    let leaveMessage;
    if (leftParticipantFbId === event.author) {
      leaveMessage = `${userName} left the group!`;
    } else {
      leaveMessage = `${userName} has been kicked`;
    }

    api.sendMessage(leaveMessage, event.threadID, event.messageID);
  }
};

module.exports.run = function({ api, event, client, __GLOBAL }) {
  // This function is not used in this event-based script
};
